<template>
   <div class="bg-background border-solid border-2 border-preto2 rounded shadow-lg shadow-preto2-500/50 p-4">
      ...
      <form @submit.prevent="salvarBanho">
...
async function salvarBanho() {
   ...
   try {
      const response = await services.banhos.save({ dados, token });
      if (response.status === 200 || response.status === 201) {
         router.push('/banhos');
      }
   } catch (error) {
      console.error("Erro no cadastro de banho ou tosa:", error);
